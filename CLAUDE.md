...
- **service/** — `SpreadsheetService` is the core: for one fee earner it calls the function set for their source list, *plus* the other set if their `usrID` is in the intersect, merges Lead+Matter rows per worksheet (no de-dup), builds the workbook, bulk-inserts archives, stores the blob, deletes the temp file. Archive writes go through `ArchiveRepository.writeForFeeEarner(...)`, which inserts all five tables **in one transaction** (all-or-nothing — a mid-write failure can't leave a partial archive). The tables share a unique clustered index `(day_run, run_id, usrID, row_number)`; **single-spreadsheet regeneration reuses the existing `run_id`** (`updateFeeEarnerRun`, `isNewRun=false`), so it passes `replaceExisting=true` and the prior rows are deleted *within the same transaction* before re-insert, avoiding a duplicate-key collision. A fresh bulk run gets a new `run_id` (IDENTITY) and passes `false`. (The blob write into `FeeEarnersRun` is a separate single statement, ordered after the archive commit.) **`output_dir` is scratch space** — the xlsx is written there, persisted as a DB blob (`FeeEarnersRun`), then deleted, so an empty `output_dir` after a run is expected; view generated sheets via the GUI's "Show Previous Runs". `generateAll`/`sendAll` use an `ExecutorService` + `CompletionService`; **per-item exceptions are caught, logged, recorded in `ProgressTracker.failures`, and the loop continues** — one bad fee earner never aborts the batch.

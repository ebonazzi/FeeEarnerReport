...
- **service/** — `SpreadsheetService` is the core: for one fee earner it calls the function set for their source list, *plus* the other set if their `usrID` is in the intersect, merges Lead+Matter rows per worksheet (no de-dup), builds the workbook, bulk-inserts archives, stores the blob, deletes the temp file. The persistence is one **transaction owned by the service** (`SpreadsheetService.persist`): it opens a `Connection` from its injected `DataSource`, sets `autoCommit=false`, calls `ArchiveRepository.writeForFeeEarner(conn, …)` (all five archive tables) **and** `RunRepository.insert/updateFeeEarnerRun(conn, …)` (the blob), then commits once — so archive rows and blob are all-or-nothing. `ArchiveRepository` and the two `RunRepository` blob writers are **transaction participants**: they take a `Connection` and never commit/rollback/close (so `ArchiveRepository` no longer holds a `DataSource`). The archive tables share a unique clustered index `(day_run, run_id, usrID, row_number)`; **single-spreadsheet regeneration reuses the existing `run_id`** (`updateFeeEarnerRun`, `isNewRun=false`), so `persist` passes `replaceExisting=true` and prior rows are deleted within the same transaction before re-insert, avoiding a duplicate-key collision. A fresh bulk run gets a new `run_id` (IDENTITY) and passes `false`. **`output_dir` is scratch space** — the xlsx is written there, persisted as a DB blob (`FeeEarnersRun`), then deleted, so an empty `output_dir` after a run is expected; view generated sheets via the GUI's "Show Previous Runs". `generateAll`/`sendAll` use an `ExecutorService` + `CompletionService`; **per-item exceptions are caught, logged, recorded in `ProgressTracker.failures`, and the loop continues** — one bad fee earner never aborts the batch.
- **excel/** — `WorkbookBuilder` orchestrates 5 `*SheetBuilder`s. `BaseRow` is a **sealed interface** permitting exactly the 5 row records; shared columns live there. Conventions enforced across all sheets (in `BaseSheetBuilder`): the **`Type` column ("Matter"/"Enquiry") is always column 0**; header row is bold + frozen + auto-filtered; date cells use `dd/MM/yyyy`. The `HEADERS` lists mirror the SQL column names *with* `[]` quoting, but `BaseSheetBuilder.displayHeader` **strips the `[]`** so spreadsheet headers are clean. **Report Date is intentionally omitted** from the spreadsheet (still archived in the DB), so base columns are indices 0–15 and each subclass's extra column is index 16. Column widths are auto-sized then **capped at 70 chars** (no wrapping). Output file name: `fe_<usrID>_<YYYYMMDD>_<runId>.xlsx`.
